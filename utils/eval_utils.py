...
def split_results_by_domain(domain_dict: dict, data: list, predictions: torch.tensor):
    ...
    assert (
        predictions.shape[0] == labels.shape[0]
    ), "The batch size of predictions and labels does not match!"
...
def eval_domain_dict(domain_dict: dict, domain_seq: list):
    ...
    domain_names = (
        domain_seq
        if all([dname in domain_seq for dname in domain_dict.keys()])
        else domain_dict.keys()
    )
    logger.info(f"Splitting the results by domain...")
    for key in domain_names:
        label_prediction_arr = np.array(
            domain_dict[key]
        )  # rows: samples, cols: (label, prediction)
    ...
    with torch.no_grad():
        for i, data in enumerate(data_loader):
            ...
            if print_every > 0 and (i + 1) % print_every == 0:
                logger.info(
                    f"#batches={i+1:<6} #samples={num_samples:<9} error = {1 - num_correct / num_samples:.2%}"
                )
