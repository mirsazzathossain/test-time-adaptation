...
vggnet = nn.Sequential(
    ...
    nn.Conv2d(
        3, 64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), padding_mode="reflect"
    ),
...
decoder = nn.Sequential(
    ...
    nn.ReLU(inplace=True),
    nn.Conv2d(
        64, 3, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), padding_mode="reflect"
    ),
    nn.Sigmoid(),
)
...
class AdaIN(nn.Module):
    ...
    def forward(self, x_content, moments_list, pretrain=False):
        ...
        rand_sample_nr = (
            torch.randint(means_style.shape[0], size=x_content.shape[:1])
            if pretrain
            else -1 * torch.arange(1, x_content.shape[0] + 1)
        )
        ...
        adain_out = (
            x_content - means_content
        ) / stds_content * stds_style + means_style
...
class TransferNet(nn.Module):
    def __init__(
        self, ckpt_path_vgg, ckpt_path_dec, data_loader, num_iters_pretrain=20000
    ):
        """
        ...
        if not os.path.isfile(ckpt_path_dec):
            ...
            self.pretrain_adain(
                final_ckpt_path=ckpt_path_dec, num_iters=num_iters_pretrain
            )
    ...
    def pretrain_adain(self, final_ckpt_path, num_iters=20000):
        ...
        moments_list = [
            [torch.tensor([], device="cuda"), torch.tensor([], device="cuda")]
            for _ in range(2)
        ]
        n_samples = 0
        ...
        with torch.no_grad():
            for images, labels in self.data_loader:
                ...
                for i_adain_layer, (means, stds) in enumerate(
                    out_adain
                ):  # iterate through the adain layers
                    moments_list[i_adain_layer][0] = torch.cat(
                        [moments_list[i_adain_layer][0], means], dim=0
                    )
                    moments_list[i_adain_layer][1] = torch.cat(
                        [moments_list[i_adain_layer][1], stds], dim=0
                    )
        ...
        for i in range(1, num_iters + 1):
            ...
            gen_img, loss_content, loss_style = self.forward(
                images=images.cuda(), moments_list=moments_list, pretrain=True
            )
            ...
            if i % 500 == 0:
                logger.info(
                    f"[{i}/{num_iters}] loss: {avg_loss / 500:.4f}, "
    ...
    def forward(self, images, moments_list=None, pretrain=False):
        ...
        if moments_list is None:
            ...
            out_encoder, means_style_enc, stds_style_enc = self.adain(
                out_encoder, moments_list=moments_list[1], pretrain=pretrain
            )
            ...
            fm11_enc, means_style_11, stds_style_11 = self.adain(
                fm11_enc, moments_list=moments_list[0], pretrain=pretrain
            )
        ...
        if self.training:
            ...
            loss_style = (
                self.mse_criterion(means_gen_11, means_style_11)
                + self.mse_criterion(means_gen_enc, means_style_enc)
                + self.mse_criterion(stds_gen_11, stds_style_11)
                + self.mse_criterion(stds_gen_enc, stds_style_enc)
            )
