...
from datasets.imagenet_subsets import (
    IMAGENET_A_MASK,
    IMAGENET_D109_MASK,
    IMAGENET_R_MASK,
    IMAGENET_V2_MASK,
...
def get_torchvision_model(model_name: str, weight_version: str = "IMAGENET1K_V1"):
    ...
    """
    assert version.parse(torchvision.__version__) >= version.parse(
        "0.13"
    ), "Torchvision version has to be >= 0.13"
    ...
    if model_name not in available_models:
        raise ValueError(
            f"Model '{model_name}' is not available in torchvision. Choose from: {available_models}"
        )
    ...
    available_weights = [
        init_name for init_name in dir(model_weights) if "IMAGENET1K" in init_name
    ]

    # check if the specified type of weights is available
    if weight_version not in available_weights:
        raise ValueError(
            f"Weight type '{weight_version}' is not supported for torchvision model '{model_name}'."
    ...
    logger.info(
        f"Successfully restored '{weight_version}' pre-trained weights"
    ...
    preprocess = transforms.Compose(
        [
            transforms.Resize(
                transform.resize_size, interpolation=transform.interpolation
            ),
...
def get_timm_model(model_name: str):
    ...
    if model_name not in available_models:
        raise ValueError(
            f"Model '{model_name}' is not available in timm. Choose from: {available_models}"
        )
...
class ResNetDomainNet126(torch.nn.Module):
    ...
    def __init__(
        ...
        self.encoder = nn.Sequential(
            ImageNormalizer((0.485, 0.456, 0.406), (0.229, 0.224, 0.225)), self.encoder
        )
...
class ZeroShotCLIP(nn.Module):
    def __init__(self, cfg, model, device, normalize):
        ...
        if self.prompt_mode in ["ensemble", "all_prompts"]:
            try:
                prompt_templates = eval(
                    f"{cfg.CORRUPTION.DATASET.split('_')[0]}_templates"
                )
            except NameError:
                logger.warning(
                    f"Could not find dataset specific prompt templates! Using ImageNet prompt templates!"
                )
        ...
        if self.prompt_mode not in ["custom", "ensemble"]:
            ...
            logger.info(
                f"Successfully restored CuPL prompts from '{cfg.CLIP.PROMPT_PATH}'"
            )
        ...
        with torch.no_grad():
            ...
            for c_name in self.class_names:
                texts = (
                    [template.format(c_name) for template in prompt_templates]
                    if self.prompt_mode != "cupl"
                    else []
                )
    ...
    def forward(self, imgs_test, return_features=False):
        ...
        if (
            ...
            or self.cfg.MODEL.ADAPTATION == "source"
            or "norm" in self.cfg.MODEL.ADAPTATION
        ):
            ...
            img_features, text_features, _ = self.model(
                imgs_test, self.tokenized_texts_all
            )
...
def get_model(cfg, num_classes: int, device: Union[str, torch.device]):
    ...
    if cfg.MODEL.USE_CLIP:
        ...
        try:
            ...
            base_model, preprocess = get_torchvision_model(
            ...
            try:
                ...
                try:
                    ...
                    logger.info(
                        f"Successfully restored model '{cfg.MODEL.ARCH}' from: {cfg.MODEL.CKPT_PATH}"
                    )
                except ValueError:
...
def split_up_model(model, arch_name: str, dataset_name: str):
    ...
    if (
        hasattr(model, "model")
        and hasattr(model.model, "pretrained_cfg")
        and hasattr(model.model, model.model.pretrained_cfg["classifier"])
    ):
        ...
        "resnet" in arch_name
        or "resnext" in arch_name
        or "wide_resnet" in arch_name
        or arch_name
        in {
            "Standard_R50",
            "Hendrycks2020AugMix",
